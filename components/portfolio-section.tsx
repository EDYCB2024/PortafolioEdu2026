...
export function PortfolioSection({ data }: PortfolioSectionProps) {
  ...
  const filteredProjects =
    activeFilter === 'all' ? data.projects : data.projects.filter((p: any) => p.category === activeFilter)
    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="flex flex-wrap gap-2 md:gap-3">
        {data.categories.map((category: string) => {
          const label = data.categoriesTrans ? data.categoriesTrans[category] : category
          return (
            <button
              key={category}
            ...
            >
              {data.categoriesTrans ? data.categoriesTrans[category as keyof typeof data.categoriesTrans] : category}
            </button>
          )
        })}
      </div>
        ...
        {filteredProjects.map((project: any, index: number) => (
          ...
          >
            ...
            <div className="absolute top-3 right-3 md:top-4 md:right-4 px-2.5 md:px-3 py-1 md:py-1.5 bg-background/90 backdrop-blur-sm border border-border rounded-lg text-xs font-medium text-accent capitalize">
              {data.categoriesTrans ? data.categoriesTrans[project.category as keyof typeof data.categoriesTrans] : project.category}
            </div>
          </div>
        ))}
      </div>
